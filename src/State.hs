...
data TrackState = TS
  { _ts_cachedTrack  :: Track -- Contains all but the last two track segments (the last two are generated every frame) [TODO, not how it works right now, but how it should work probably]
...
trackStateFromWaypoints waypoints =
  ...
  in TS track (bimap revList revList corners) (revList waypoints)

emptyEditorTrackState :: TrackState
emptyEditorTrackState = TS [] (revList [] , revList []) (Rev [(zeroVec, 100)])
